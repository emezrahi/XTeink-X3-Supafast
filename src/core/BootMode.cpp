...
BootMode detectBootMode() {
  ...
  // No pending transition - if we have a previously opened book, boot straight
  // into reader mode.
  if (core.settings.lastBookPath[0] != '\0' && SdMan.exists(core.settings.lastBookPath)) {
    LOG_INF(TAG, "Startup: opening last book %s", core.settings.lastBookPath);

    // Set up cached transition for reader mode
    cachedTransition.magic = ModeTransition::MAGIC;
    cachedTransition.mode = BootMode::READER;
    cachedTransition.returnTo = ReturnTo::FILE_MANAGER;
    strncpy(cachedTransition.bookPath, core.settings.lastBookPath, sizeof(cachedTransition.bookPath) - 1);
    cachedTransition.bookPath[sizeof(cachedTransition.bookPath) - 1] = '\0';
    transitionCached = true;
...
void showTransitionLoadingAnimation(uint8_t loops, uint16_t frameDelayMs) {
  const Theme& theme = THEME_MANAGER.current();
  const char* frames[] = {"LOADING.", "LOADING..", "LOADING..."};
  constexpr size_t frameCount = sizeof(frames) / sizeof(frames[0]);
  const int screenHeight = renderer.getScreenHeight();
  const int lineHeight = renderer.getLineHeight(theme.uiFontId);
  const int y = screenHeight / 2 - lineHeight / 2;

  for (uint8_t loop = 0; loop < loops; ++loop) {
    for (size_t i = 0; i < frameCount; ++i) {
      renderer.clearScreen(theme.backgroundColor);
      renderer.drawCenteredText(theme.uiFontId, y, frames[i], theme.primaryTextBlack, EpdFontFamily::BOLD);
