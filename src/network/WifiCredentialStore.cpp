...
// Obfuscation key - "Pixelpaper" in ASCII (not cryptographic, just prevents casual reading)
...
bool WifiCredentialStore::saveToFile() const {
  ...
  FsFile file;
  if (!SdMan.openFileForWrite("WCS", PIXELPAPER_WIFI_FILE, file)) {
...
bool WifiCredentialStore::loadFromFile() {
  FsFile file;
  if (!SdMan.openFileForRead("WCS", PIXELPAPER_WIFI_FILE, file)) {
    return false;
  }
