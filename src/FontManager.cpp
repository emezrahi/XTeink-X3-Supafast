...
void FontManager::init(GfxRenderer& r) {
  ...
  // Ensure custom fonts directory exists on SD card for first-time setup.
  if (!SdMan.exists(CONFIG_FONTS_DIR) && !SdMan.mkdir(CONFIG_FONTS_DIR)) {
    LOG_ERR(TAG, "Failed to create fonts directory: %s", CONFIG_FONTS_DIR);
  }
}
