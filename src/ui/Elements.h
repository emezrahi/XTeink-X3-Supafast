...
void fileEntry(const GfxRenderer& r, const Theme& t, int y, const char* name, bool isDir, bool selected,
               uint8_t progressState = 0, uint8_t progressPct = 0);
...
struct ReaderStatusBarData {
  ...
  int overallCurrentPage = 0;  // Overall book page position (for progress bar)
  int overallTotalPages = 0;   // Overall book total pages (for progress bar)
};
