...
void render(const GfxRenderer& r, const Theme& t, const ConfirmDialogView& v) {
  ...
  // Title (bold, uppercase, left aligned)
  char upperTitle[64];
  size_t idx = 0;
  for (; v.title[idx] != '\0' && idx + 1 < sizeof(upperTitle); idx++) {
    char c = v.title[idx];
    if (c >= 'a' && c <= 'z') c = static_cast<char>(c - ('a' - 'A'));
    upperTitle[idx] = c;
  }
  upperTitle[idx] = '\0';
  r.drawText(t.uiFontId, 20, top - 40, upperTitle, t.primaryTextBlack, EpdFontFamily::BOLD);
