...
struct FileListView {
  ...
  // Reading-progress states for non-directory entries
  enum class ReadProgress : uint8_t {
    Unknown   = 0,  // Directories or no cache data yet
    Unread    = 1,  // No saved position (never opened or at page 0)
    InProgress = 2, // Partially read; use progressPct (0–99)
    Finished  = 3,  // Reached the last chapter / page
  };
  ...
  struct FileEntry {
    char name[NAME_LEN];
    bool isDirectory;
    ReadProgress readProgress = ReadProgress::Unknown;
    uint8_t progressPct = 0;  // 0–99, meaningful only when readProgress == InProgress
  };
