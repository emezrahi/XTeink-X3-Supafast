...
struct SettingsMenuView {
  ...
  ButtonBar buttons{"Close", "Open", "Up", "Down"};
  int8_t selected = 0;
...
struct CleanupMenuView {
  ...
  ButtonBar buttons{"Back", "Run", "Up", "Down"};
  int8_t selected = 0;
  bool needsRender = true;
...
struct ReaderSettingsView {
  ...
  static constexpr int MAX_FONTS = 24;
  static const SettingDef DEFS[SETTING_COUNT];

  ButtonBar buttons{"Back", "Toggle", "Up", "Down"};
  ...
  // Font selection state (loaded from /fonts)
...
struct DeviceSettingsView {
  ...
  static constexpr const char* const STARTUP_VALUES[] = {"Last Document", "Library"};
...
struct ConfirmDialogView {
  ...
  ButtonBar buttons{"Close", "Select", "Left", "Right"};
