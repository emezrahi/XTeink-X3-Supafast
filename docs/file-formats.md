...
This document describes the binary cache formats used by Pixelpaper for EPUB, TXT, Markdown, FB2, and HTML files.

## TXT Cache Files

TXT files use a simple cache format stored in `.pixelpaper/txt_<hash>/`.
...
Markdown files (`.md`, `.markdown`) use a cache format stored in `.pixelpaper/md_<hash>/`.
...
FB2 (FictionBook 2.0) files use a cache format stored in `.pixelpaper/fb2_<hash>/`.
...
HTML files (`.html`, `.htm`) use a cache format stored in `.pixelpaper/html_<hash>/`. HTML is parsed using the same pipeline as EPUB chapters (HTML5 normalization → Expat XML → page layout).
...
EPUB files use a more complex cache format stored in `.pixelpaper/epub_<hash>/`.
