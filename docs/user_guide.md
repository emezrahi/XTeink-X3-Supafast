# Pixelpaper User Guide

Welcome to the **Pixelpaper** firmware. This guide outlines the hardware controls, navigation, and reading features of
the device.

> Project lineage: Pixelpaper continues earlier community firmware work for the Xteink X4 under the MIT license. See [LICENSE](../LICENSE) for legal terms and attribution.
...
The Home Screen displays the title "Pixelpaper" at the top with a **battery indicator** in the top-right corner.
...
> - `config` — Pixelpaper configuration files
> - `XTCache` — XTC format cache
> - `sleep` — Custom sleep screen images
> - `.pixelpaper` — Internal cache (dot-prefix hidden by default)
...
Calibre Wireless allows you to send books directly from **Calibre** (the popular ebook management software) to your Pixelpaper Reader over WiFi. This is the fastest way to transfer books if you already use Calibre.
...
3. Calibre will automatically discover your Pixelpaper Reader
4. Your device appears as "Pixelpaper Reader" (or your custom name)
