...
Pixelpaper Reader supports custom fonts for reading. Fonts are converted to a proprietary `.epdfont` format optimized for e-paper displays.
...
Pixelpaper ensures you can always read your books, even if a custom font fails:
...
IBM's Arabic typeface from the Plex family. A modern, clean sans-serif design with excellent Arabic presentation forms coverage (140/144 in Forms-B), making it highly compatible with Pixelpaper's Arabic shaper. Provides a refined reading experience for Arabic content.
...
For CJK texts, Pixelpaper uses external `.bin` format fonts that are streamed from the SD card due to their large size. The `.bin` format uses direct codepoint indexing (1-bit bitmap, MSB first) for the full BMP range (U+0000-U+FFEF).
