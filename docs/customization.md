...
This guide explains how to create custom themes and add custom fonts to Pixelpaper.

---

## Custom Themes

Pixelpaper supports user-customizable themes stored on the SD card. Themes control colors, layout options, and fonts.
...
```ini
# Pixelpaper Theme Configuration
...
- Reader (small): `/fonts/noto-serif-14/`
- Reader (medium): `/fonts/noto-serif-16/`
- Reader (large): `/fonts/noto-serif-18/`
...
Pixelpaper supports loading custom fonts from the SD card. Fonts must be pre-converted to the `.epdfont` binary format.
...
uv run scripts/fontconvert.py my-font -r Font.ttf --2bit -o /Volumes/SDCARD/fonts/
...
Copy the entire folder(s) to `/fonts/` on your SD card.
...
Each font family name must match a directory name under `/fonts/`. You can use the same font for all sizes, or different fonts for each size.
...
2. Copy the corresponding font folders to `/fonts/` on your SD card
