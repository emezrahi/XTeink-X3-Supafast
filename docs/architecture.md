...
This document describes the internal architecture and subsystems of Pixelpaper.

## Overview

Pixelpaper is organized around a **state machine** architecture with **singleton managers** and **content providers** for multi-format ebook support. The system is optimized for the ESP32-C3's ~380KB RAM constraint.
...
Pixelpaper uses a finite state machine (FSM) with pre-allocated state instances. This avoids heap allocation during state transitions, preventing memory fragmentation.
...
To maximize available RAM in reader mode, Pixelpaper implements a dual-boot system:
...
Cache location: `/.pixelpaper/<format>_<hash>/progress.bin`

---

## Memory Management

The ESP32-C3 has ~380KB usable RAM with ~100-150KB available after system overhead. Pixelpaper employs several strategies:
...
Pixelpaper uses the **Knuth-Plass algorithm** for optimal line breaking, the same algorithm used by TeX. This produces higher-quality justified text than greedy algorithms.
...
Pixelpaper supports multiple writing systems through script detection and specialized rendering.
...
Pixelpaper uses a view-based UI architecture with reusable elements and state-driven rendering.
...
reader-test --cache-dump /path/to/.pixelpaper/epub_<hash>/
...
- **`PixelpaperSettings.h`** — User preferences
