...
bool EpubChapterParser::parsePages(const std::function<void(std::unique_ptr<Page>)>& onPageComplete, uint16_t maxPages,
  ...
  const std::string& cachePath = epub_->getCachePath();
  char indexBuf[16];
  const int indexLen = snprintf(indexBuf, sizeof(indexBuf), "%d", spineIndex_);
  const char* idx = (indexLen > 0) ? indexBuf : "0";
  const size_t idxLen = (indexLen > 0) ? static_cast<size_t>(indexLen) : 1;

  tmpHtmlPath_.clear();
  tmpHtmlPath_.reserve(cachePath.size() + 12 + idxLen);
  tmpHtmlPath_.append(cachePath);
  tmpHtmlPath_.append("/.tmp_");
  tmpHtmlPath_.append(idx, idxLen);
  tmpHtmlPath_.append(".html");
  ...
  normalizedPath_.clear();
  normalizedPath_.reserve(cachePath.size() + 13 + idxLen);
  normalizedPath_.append(cachePath);
  normalizedPath_.append("/.norm_");
  normalizedPath_.append(idx, idxLen);
  normalizedPath_.append(".html");
